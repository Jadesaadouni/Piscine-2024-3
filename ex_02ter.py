...
def display_french_NP():
    french_laureates = []  # Une liste pour stocker les lauréats français

    # Parcourez les documents de la collection "laureates"
    for document in collection.find():
        laureates = document.get('laureates', [])  # Obtenez la liste des lauréats

        # Parcourez les lauréats dans le document
        for laureate in laureates:
            bornCountry = laureate.get('bornCountry', '')  # Obtenez le pays de naissance du lauréat

            # Vérifiez si le pays de naissance est "France"
            if bornCountry == 'France':
                french_laureates.append({
                    'firstname': laureate.get('firstname', ''),
                    'surname': laureate.get('surname', ''),
                    'year': laureate['prizes'][0]['year'],  # Obtenez l'année du prix Nobel
                    'category': laureate['prizes'][0]['category']  # Obtenez la catégorie du prix Nobel
                })

    # Affichez les lauréats français et leurs prix Nobel
    for laureate in french_laureates:
        print(f"Firstname: {laureate['firstname']}")
        print(f"Surname: {laureate['surname']}")
        print(f"Year: {laureate['year']}")
        print(f"Category: {laureate['category']}")
        print()

#Appelez la fonction pour afficher les lauréats français
display_french_NP()
